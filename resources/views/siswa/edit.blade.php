    ...
    <main class="main-content  mt-0">
        ...
        <div class="row">
            ...
            <div class="card z-index-0 fadeIn3 fadeInBottom">
                ...
                <div class="card-body">
                <form role="form" action="{{ route('datasiswa.update',$siswa->id) }}" class="text-start" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    @enderror
                    <div class="input-group input-group-outline my-3">
                    <input type="email" name="email" value="{{ $siswa->email }}" class="form-control" id="dengan-rupiah">
                    </div>
                    ...
                    <div class="input-group input-group-outline my-3">
                        <select name="id_instansi" class="form-select form-select-outline my-3">
                            ...
                            @foreach($instansi as $key => $value)
                                <option value="{{ $key }}" {{ $key == $siswa->id_instansi ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="input-group input-group-outline my-3">
                        <select name="id_pembimbing" class="form-select">
                            ...
                            @foreach($pembimbing as $key => $value)
                                <option value="{{ $key }}" {{ $key == $siswa->id_pembimbing ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
